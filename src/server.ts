...
interface GradioOptions {
  inpaint_prompt?: string;
  box_threshold?: number;
  text_threshold?: number;
  iou_threshold?: number;
  inpaint_mode?: "merge" | "first";
  scribble_mode?: "merge" | "split";
  openai_api_key?: string;
}

type TaskType = "det" | "seg" | "inpainting";
...
class ImageProcessor {
   ...
   * 创建透明遮罩
   */
  static createTransparentMask(): string {
    const transparentPng =
      "iVBORw0KGgoAAAANSUhEUgAAAAoAAAAKCAYAAACNMs+9AAAABHNCSVQICAgIfAhkiAAAAAlwSFlzAAAAdgAAAHYBTnsmCAAAABl0RVh0U29mdHdhcmUAd3d3Lmlua3NjYXBlLm9yZ5vuPBoAAAAQSURBVBiVY2AYBaNgFIwAAATgAAFPbxqYAAAAAElFTkSuQmCC";
   ...
   * 下载文件到本地
   */
  static async downloadFile(fileUrl: string, outputPath: string): Promise<string> {
   ...
   */
  static async fetchUrlAsBase64(url: string): Promise<string> {
   ...
   * 加载图片为 base64 格式
   */
  static async loadImageAsBase64(imageUrlOrPath: string): Promise<string> {
...
class GradioClient {
   ...
   * 处理 Gradio 结果并保存文件
   */
  static async processResult(result: any, baseUrl: string, outputDir: string): Promise<string[]> {
    const saved: string[] = [];
    
    if (!result?.data || !Array.isArray(result.data)) return saved;
...
class ResultFormatter {
   ...
   * 格式化保存的文件路径为 file:// 格式
   */
  static formatFilePaths(savedFiles: string[]): string {
    return savedFiles
   ...
   * 创建成功响应
   */
  static createSuccessResponse(savedFiles: string[]): any {
   ...
   * 创建错误响应
   */
  static createErrorResponse(message: string): any {
    return {
      ...
      "detect",
      {
        ...
        inputSchema: {
          ...
          text_threshold: zeroToOneSchema.optional().describe("短语抽取阈值，默认0.25"),
             ...
             async ({ image, text_prompt, box_threshold, text_threshold }) => {
         const baseUrl = GRADIO_ENDPOINT_URL;
      ...
      "inpaint",
      {
        ...
        inputSchema: {
          ...
          inpaint_mode: z.enum(["merge", "first"]).optional().describe("掩码合并或只用第一张，默认 merge"),
             ...
             async ({ image, text_prompt, inpaint_prompt, box_threshold, text_threshold, inpaint_mode }) => {
         const baseUrl = GRADIO_ENDPOINT_URL;
        ...
        messages: [
          {
            ...
            content: {
              type: "text",
              text:
                "You are a parameter generator for an AI vision MCP tool.\n" +
